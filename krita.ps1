...
$name = "krita"
$portable = $true
$version = (Invoke-WebRequest https://download.kde.org/stable/krita/).Content | Select-String -Pattern 'href="[0-9].[0-9].[0-9]/"' -AllMatches | Select-String '[0-9].[0-9].[0-9]'  -AllMatches | ForEach-Object { $_.Matches.Value } | Select-String -Pattern '[0-9].[0-9].[0-9]' | ForEach-Object {$_.Matches.Value} | Sort-Object -Descending | Select-Object -first 1
$source = "https://download.kde.org/stable/krita/$version/krita-x64-$version.zip"
$sourcetype = "resolver"
$executable = "$path\$name\$version\bin\krita.exe"
$uninstaller = ""
$dependencies = @()
