...
$name = "aria2"
$portable = $true
$version = (Invoke-WebRequest "https://api.github.com/repos/aria2/aria2/releases" | ConvertFrom-Json)[0].tag_name -replace 'release-'
$source = "https://github.com/aria2/aria2/releases/download/release-$version/aria2-$version-win-64bit-build1.zip"
$sourcetype = "resolver"
$executable = "$path\$name\$version\aria2c.exe"
$uninstaller = ""
$dependencies = @()
...
if($cmd -eq "install")
{
    ...
    Write-Host "If a popup appears, press 'Yes' to add a firewall rule for aria2."

    $line = "New-NetFirewallRule -DisplayName 'aria2c/denget' -Direction Inbound -Action Allow -EdgeTraversalPolicy DeferToUser -Protocol TCP -LocalPort Any -Profile Private -Program '$executable'"
    $line > "$dgpath\temp\ariascript.ps1"
    $line = "New-NetFirewallRule -DisplayName 'aria2c/denget' -Direction Inbound -Action Allow -EdgeTraversalPolicy DeferToUser -Protocol UDP -LocalPort Any -Profile Private -Program '$executable'"
    $line >> "$dgpath\temp\ariascript.ps1"

    Start-Sleep -s 2
    Start-Process powershell.exe -Verb RunAs -ArgumentList "-noprofile -File $dgpath\temp\ariascript.ps1" -WindowStyle Hidden -Wait

    Remove-Item "$dgpath\temp\ariascript.ps1"
...
if($cmd -eq "uninstall")
{
    ...
    Write-Quiet "aria2 is used by denget to support bittorrent sources and to download apps faster."
    Write-Quiet "Uninstalling it will disable these features. See '" -n; Write-Quiet "denget features" -f m -n; Write-Quiet "' for more information."
    ...
    Write-Host "If a popup appears, press 'Yes' to remove a firewall rule for aria2."

    $line = "Remove-NetFirewallRule -DisplayName 'aria2c/denget'"
    $line > "$dgpath\temp\ariascript.ps1"
