...
$portable = $true
$version = "3.21"
$source = "https://github.com/pbatard/rufus/releases/download/v3.21/rufus-3.21p.exe"
$sourcetype = "hardlink"
$executable = "$path\$name\$version\rufus.exe"
$uninstaller = ""
$dependencies = @()
...
if($cmd -eq "install")
{
    ...
    Copy-Item "$dgpath\temp\$name.exe" "$path\$name" -Force
